 // Image.cc adapted for use in bbtools 03-1999 by John Kennis
//	
//
// Original notice:
//
...
XImage *BImage::renderXImage(void) {
  ...
  if (control->doDither()) {
    short er, eg, eb, *ord, *og, *ob, *nor, *nog, *nob, *por, *pog, *pob;
    unsigned short *ort, *ogt, *obt;
    
    control->getDitherBuffers(width + 2, &ord, &og, &ob, &nor, &nog, &nob,
			      &ort, &ogt, &obt);
    if ((! ord) || (! og) || (! ob) || (! nor) || (! nog) || (! nob) ||
	(! ort) || (! ogt) || (! obt)) {
      fprintf(stderr,
	...
	*(nob + x) = *(blue + i);
      ...
      for (x = 0; x < width; x++) {
	if (*(ord + x) > 255) *(ord + x) = 255;
	else if (*(ord + x) < 0) *(ord + x) = 0;
...
BImageControl::BImageControl(Basewindow *bb) {
  blackbox = bb;
  display = blackbox->getDisplay();
  screen_depth = blackbox->getDepth();
  ...
  switch (blackbox->getVisual()->c_class) {
  case TrueColor:
    {
      int i;

      // compute tables for red channel
      unsigned long mask = blackbox->getVisual()->red_mask, emask = mask;
	...
	rerr_table[i] = ((rmask_table[i] << 8) / (emask >> red_offset));
      ...
      emask = mask = blackbox->getVisual()->green_mask;
	...
	gerr_table[i] = ((gmask_table[i] << 8) / (emask >> green_offset));
      ...
      emask = mask = blackbox->getVisual()->blue_mask;
